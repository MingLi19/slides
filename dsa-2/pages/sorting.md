...
<!--
[click]
时间复杂度小。对于大数据量的情况, 运行效率显得尤为重要。
[click]
顾名思义, 原地排序通过在原数组上直接操作实现排序, 无须借助额外的辅助数组, 从而节省内存。通常情况下, 原地排序的数据搬运操作较少, 运行速度也更快。
[click]
稳定排序算法在完成排序后, 相等元素在数组中的相对顺序不发生改变。稳定性对于排序后数据的处理非常重要, 例如对姓名、年龄等数据进行排序时, 保持相等元素的相对位置不变。
[click]
自适应排序算法能够利用输入数据已有的顺序信息来减少计算量, 达到更优的时间效率。自适应排序算法的最佳时间复杂度通常优于平均时间复杂度。
[click]
基于比较的排序算法依赖比较运算符（<、>、=）来判断元素的相对顺序。在排序算法中, 比较运算符是最基本的操作, 因此基于比较的排序算法是最常见的排序算法。
-->
---
...
<!--
已排序区间在图中为灰色格子，元素逐渐增多, 未排序区间为绿色格子，元素逐渐减少。循环往复直至未排序区间为空, 整个数组排序完成。
-->
---
...
<!--
后面我们将学习到时间复杂度为 $O(nlogn)$ 的排序算法, 如快速排序、归并排序等。但在数据量较小的情况下, 插入排序通常更快。
这个结论与线性查找和二分查找的适用情况的结论类似。快速排序这类 $O(nlogn)$ 的排序算法, 往往包含更多单元计算操作。而在数据量较小时, $O(n^2)$ 和 $O(nlogn)$ 的数值比较接近, 复杂度不占主导地位。每轮中的单元操作数量起到决定性作用。
